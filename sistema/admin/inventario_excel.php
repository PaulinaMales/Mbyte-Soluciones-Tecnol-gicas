...
$archivo="Reporte_Inventario_".$fecha_reporte.".xls";
...
$objPHPExcel->getActiveSheet()->setCellValue('A1','Productos Registrados '.$numpe);
$objPHPExcel->getActiveSheet()->setCellValue('A2', " #.");
$objPHPExcel->getActiveSheet()->setCellValue('B2', " Item ");
$objPHPExcel->getActiveSheet()->setCellValue('C2', " Precio ");
$objPHPExcel->getActiveSheet()->setCellValue('D2', " Precio Venta ");
$objPHPExcel->getActiveSheet()->setCellValue('E2', " Unidades ");
$objPHPExcel->getActiveSheet()->setCellValue('F2', " Fecha Ingreso ");
$objPHPExcel->getActiveSheet()->freezePane('A3');//fila fija
   $i=1;
   $c=3;
   
while($row=mysqli_fetch_object($sqli)){
		  $objPHPExcel->getActiveSheet()->getStyle('A'.$c.':F'.$c)->applyFromArray($styleThickBrownBorderOutline);	
		  $objPHPExcel->getActiveSheet()->setCellValue('A'.$c, $i);
		  $objPHPExcel->getActiveSheet()->setCellValue('B'.$c, $row->item);
          $objPHPExcel->getActiveSheet()->setCellValue('C'.$c, sprintf("%01.2f",$row->costo));
          $objPHPExcel->getActiveSheet()->setCellValue('D'.$c, sprintf("%01.2f",$row->precio));
		  $objPHPExcel->getActiveSheet()->setCellValue('E'.$c, $row->unidades);
		  $fecha_1=explode(" ",$row->fecha_creacion);
		  $fecha=explode("-",$fecha_1[0]);
		  $objPHPExcel->getActiveSheet()->setCellValue('F'.$c, $mes[$fecha[1]-1]." ".$fecha[2].", ".$fecha[0]);
		  $i=$i+1;
		  $c=$c+1;
	}//fin while($row
//estilos en el archivo
$objPHPExcel->getActiveSheet()->mergeCells('A1:F1');//unir celdas
...
$objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray($styleThickBrownBorderOutline);
$objPHPExcel->getActiveSheet()->getStyle('A3:F'.$c)->applyFromArray($styleThickBrownBorderOutline2);
